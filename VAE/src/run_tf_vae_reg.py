...
from utils import plot_fig_recons, get_classification_report, plot_zmeans, plot_fig_loss
from utils import plot_2d_variation, plot_true_prediction
...
if __name__ == "__main__":
    ...
    # plot_zmeans(z_means=x_test_latent, y_test=y_test, filename=config['fig_zmeans'])
    # plot_2d_variation(model=[encoder, decoder], n=16, config=config)
    # ----------------------------
    # - map to parameters space  -
    # ----------------------------

    x_train_latent, _, _ = encoder.predict(x_train)
    x_test_latent, _, _ = encoder.predict(x_test)
    ...
    if config['data_type'].endswith('reg'):
        ...
        if config['nn_type'] == 'dense':
            encoder_zmeans = vae.build_dense_encoder_zmeans(input_shape=(x_train_latent.shape[1:]), latent_dim=2)
        elif config['nn_type'] == 'cnn':
            encoder_zmeans = vae.build_dense_encoder_zmeans(input_shape=(x_train_latent.shape[1:]), latent_dim=2)
    ...
    # if config['nn_type'] == 'dense':
    #     encoder_cls = vae.build_dense_encoder_cls(input_shape=(x_train_latent.shape[1:]), num_class=num_class)
    # elif config['nn_type'] == 'cnn':
    #     encoder_cls = vae.build_dense_encoder_cls(input_shape=(x_train_latent.shape[1:]), num_class=num_class)
